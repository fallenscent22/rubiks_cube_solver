...
import Rubiks_Cube_Solver.cube_model as cube
...
import Rubiks_Cube_Solver.move_optimizer
...
class TestVec3(unittest.TestCase):
    ...
    def test_Vec3_constructor(self):
        self.assertEqual(self.p.x, 1)
        self.assertEqual(self.p.y, 2)
        self.assertEqual(self.p.z, 3)

    def test_Vec3_count(self):
        self.assertEqual(self.r.count(2), 2)
        self.assertEqual(self.r.count(3), 1)
        self.assertEqual(self.r.count(5), 0)
        self.assertEqual(Vec3(9, 9, 9).count(9), 3)

    def test_Vec3_eq(self):
        pp = Vec3(self.p.x, self.p.y, self.p.z)
        self.assertEqual(self.p, pp)
        self.assertTrue(self.p == pp)
        self.assertTrue(self.p == (1, 2, 3))
        self.assertTrue(self.p == [1, 2, 3])

    def test_Vec3_neq(self):
        Vec3s = [Vec3(1, 2, 3), Vec3(1, 2, 4), Vec3(1, 3, 3), Vec3(2, 2, 3)]
        for i in range(len(Vec3s)):
            for j in range(i + 1, len(Vec3s)):
                self.assertNotEqual(Vec3s[i], Vec3s[j])
                self.assertTrue(Vec3s[i] != Vec3s[j])
                self.assertFalse(Vec3s[i] == Vec3s[j])

    def test_Vec3_add(self):
        self.assertEqual(self.p + self.q, Vec3(3, 7, 12))

    def test_Vec3_iadd(self):
        self.p += self.q
        self.assertEqual(self.p, Vec3(3, 7, 12))

    def test_Vec3_sub(self):
        self.assertEqual(self.p - self.q, Vec3(-1, -3, -6))

    def test_Vec3_isub(self):
        self.p -= self.q
        self.assertEqual(self.p, Vec3(-1, -3, -6))

    def test_Vec3_scale(self):
        self.assertEqual(self.p * 3, Vec3(3, 6, 9))

    def test_Vec3_dot_product(self):
        self.assertEqual(self.p.dot(self.q), 39)

    def test_Vec3_cross_produce(self):
        self.assertEqual(self.p.cross(self.q), Vec3(3, -3, 1))
    ...
    def test_from_tuple(self):
        self.assertEqual(Vec3((1, 2, 3)), self.p)
    ...
    def test_Vec3_repr(self):
        self.assertEqual(repr(self.p), "Vec3(1, 2, 3)")

    def test_Vec3_iter(self):
        ii = iter(self.p)
...
class TestMatrix(unittest.TestCase):
    ...
    def test_matrix_Vec3_mul(self):
        self.assertEqual(self.A * self.p, Vec3(14, 32, 50))
    ...
    def test_full_cube_rotation_optimization(self):
        for cw, cc in (('X', 'Xi'), ('Y', 'Yi'), ('Z', 'Zi')):
            for moves in ([cc, cw], [cw, cc]):
                Rubiks_Cube_Solver.move_optimizer.apply_no_full_cube_rotation_optimization(moves)
        ...
        for cw, cc in (('Z', 'Zi'),):
            ...
            actual = list(moves)
            Rubiks_Cube_Solver.move_optimizer.apply_no_full_cube_rotation_optimization(actual)
